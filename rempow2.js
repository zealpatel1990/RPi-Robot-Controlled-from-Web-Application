...
app.use(express.static('/home/pi/Desktop/rem.v1'));
...
//	if (msg>0){lf.pwmWrite(Math.round(Math.pow(2,msg/31.875)-1));} range is [-255,255]
//	if (msg>0){lf.pwmWrite(ls);}
//	else if (msg<0){lb.pwmWrite(-ls);} 
	if (msg>0){lf.pwmWrite(Math.round(Math.pow(2,msg/2)-1));}
	else if (msg<0){lb.pwmWrite(Math.round(Math.pow(2,-msg/2)-1));}
	else if (msg==0){lf.digitalWrite(0);lb.digitalWrite(0);}
    console.log('left: ',Math.round(Math.pow(2,msg/2)-1));
  });
  socket.on('right_slider', function(msg){
  	var rs = msg;
//    if (msg>0){rf.pwmWrite(rs);}
//	else if (msg<0){rb.pwmWrite(-rs);}
	if (msg>0){rf.pwmWrite(Math.round(Math.pow(2,msg/2)-1));}
	else if (msg<0){rb.pwmWrite(Math.round(Math.pow(2,-msg/2)-1));}
	else if (msg==0){rf.digitalWrite(0);rb.digitalWrite(0);}
    console.log('right: ',Math.round(Math.pow(2,msg/2)-1));
  });
